...
class CircleComposite extends Circle {
    ...
    mergeArcs() {
        ...
        for (let i = 0; i < this.arcs.length; i++) {
            ...
            angle_points.push(arc.radians1);
            angle_points.push(arc.radians2);
        }
        angle_points = Calc.sortUniqueNum(angle_points);
        let angle1 = new Angle(angle_points[0], angle_points[angle_points.length - 1]);
        if (!angle1.isDVA_PI)
        ...
        for (let p = 1; p < angle_points.length; p++) {
            ...
            // let arc_1 = new Arc(this.x, this.y, this.radius, new Angle(p1, p2), this.ccw);
