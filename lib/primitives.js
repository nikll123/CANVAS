...
class Text extends ObjGraf {
    ...
    render(ctx, color = DEFAULT_COLOR, font = '10px sans-serif', bl = 'bottom') {
        this._render(ctx, color, font, bl, RENDER_NORMAL);
    }

    draft(ctx, color = DEFAULT_COLOR, font = '10px sans-serif', bl = 'bottom') {
        this._render(ctx, color, font, bl, RENDER_DRAFT);
    }

    _render(ctx, color, font, bl, draft) {
        ctx.save();
        ctx.font = font;
        ctx.fillStyle = color;
        ctx.textBaseline = bl;
...
class Angle extends ObjGraf {
    ...
    normalize() {
        const oldBegin = this.rayBegin.value;
        this.rayBegin.normalize();
        const diff = this.rayBegin.value - oldBegin;
        this.rayEnd.add(diff);
    }
    ...
    normalizeEnd(ccw = CW) {
        ...
        if (ccw == CCW) {
            if (this.begin < this.end) {
                k = 1 + Math.floor((this.end - this.begin) / DVA_PI);
                this.end = this.end - k * DVA_PI;
            }
        }
        else {
            if (this.begin > this.end) {
                k = 1 + Math.floor((this.begin - this.end) / DVA_PI);
                this.end = this.end + k * DVA_PI;
            }
        }
        this.normalize();
