...
function drawArrowArc(ctx, x, y, r, a1, ccw, size = 8) {
    ...
    if (ccw)
        k = -1;
    a1 = a1 + k * PI / 2;
    drawArrow(ctx, endX, endY, a1, size);
}

function drawArrow(ctx, x, y, a1, size = 8) {
    let a2 = PI / 8;
    let x1 = x - size * Math.cos(a1 + a2);
    let y1 = y - size * Math.sin(a1 + a2);
    let x2 = x - size * Math.cos(a1 - a2);
    let y2 = y - size * Math.sin(a1 - a2);
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x, y);
    ctx.lineTo(x2, y2);
