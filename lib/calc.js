...
class Calc {
    ...
    static crossCircleLine(c, l) {
        ...
        if (h >= c.radius) {
            ...
            let angle_base = new Angle(0, angle_l);
            angle_base.add(PI / 2);
            let k = Math.tan(angle_l);
            let a = l.y - k * l.x;      //      y = k*x + a
            let lcy = k * c.x + a;
            if (lcy < c.y)
                angle_base.add(PI);
            a1 = Math.acos(h / c.radius);
            let a_tmp1 = new Angle(0, angle_base.value);
            a_tmp1.substract(a1);
            let a_tmp2 = new Angle(0, angle_base.value);
            a_tmp2.add(a1);
