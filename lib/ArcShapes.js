...
class CircleCombo extends Circle {
    ...
    render(ctx, lineWidth, colorStroke = DEFAULT_COLOR, fillColor, connectParts = false) {
        this._render(ctx, lineWidth, colorStroke, fillColor, connectParts, false)
    }
    draft(ctx, lineWidth = 1, colorStroke = DRAFT_COLOR, fillColor, connectParts = false) {
        this._render(ctx, lineWidth, colorStroke, fillColor, connectParts, DRAFT)
    }
    ...
    _render(ctx, lineWidth, colorStroke = DEFAULT_COLOR, fillColor, connectParts, draft) {
        if (lineWidth > 0) {
            ctx.save();
            this.build(ctx, connectParts);
            ctx.lineWidth = lineWidth;
            ctx.strokeStyle = colorStroke;
            if (draft)
                ctx.setLineDash(DRAFT_LINE_PATTERN_5);
