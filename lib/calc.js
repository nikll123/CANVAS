...
class Calc {
    ...
    static sortUniqueNum(array, descend = false) {
        let k = 1;
        if (descend)
            k = -1;
        return array.sort(function (a, b) { return k * (a - b); }).filter(function (el, i, a) { return i === a.indexOf(el) })
    }
    ...
    static getAltitudeTriangle(Ax, Ay, Bx, By, Cx, Cy) {
        /*        C
                 /|\
                / | \
               /  |  \
             ...
             A    H    B
        */
        let CH, Hx, Hy, XCH_angle, pos;
        let XAB_angle = Calc.getAngle(Ax, Ay, Bx, By);
        ...
        if (CH == 0) {
            ...
            if (CH > 0) {
                k = -1
                pos = LINEPNTPOS_RIGHT;
            }
            else {
                k = 1
                pos = LINEPNTPOS_LEFT;
            }
            XCH_angle = XAB_angle + k * PI / 2;
            CH = Math.abs(CH);
        }
        return [CH, Hx, Hy, XCH_angle, pos];
    }
