...
class CalcCross {
    ...
    static arcCross(ctx, arc1, arc2) {
        ...
        if (distance > r1 + r2) {
            ...
            let arc2_begIsOn = arc2.isPointInStroke(ctx, crossArc2.beginX, crossArc2.beginY);
            let arc2_endIsOn = arc2.isPointInStroke(ctx, crossArc2.endX, crossArc2.endY);
            ...
            if (arc1_begIsOn && arc1_endIsOn) {
                ...
                if (arc1.isCircle) {
                    ...
                    [a11, a12] = arc1.splitXY(ctx, crossArc1.beginX, crossArc1.beginY);
                    [a12, a13] = a12.splitXY(ctx, crossArc1.endX, crossArc1.endY);
                    resArcs1.push(a11);
                    resArcs1.push(a12);
                    resArcs1.push(a13);
                }
                resArcs2 = [];
                let a21, a22, a23;
                [a21, a22] = arc2.splitXY(ctx, crossArc1.endX, crossArc1.endY);
                // arc2.render(ctx, 6,'#0000FF');
                // a22.render(ctx, 3,'#00FF00');
                [a22, a23] = a22.splitXY(ctx, crossArc1.beginX, crossArc1.beginY);
                resArcs2.push(a21);
                resArcs2.push(a22);
                resArcs2.push(a23);
            }
                ...
                let x, y, angval1, angval2;
                if (arc1_begIsOn) {
                    [x, y] = [crossArc1.beginX, crossArc1.beginY];
                    angval1 = crossArc1.begin;
                }
                else {
                    [x, y] = [crossArc1.endX, crossArc1.endY];
                    angval1 = crossArc1.end;
                }
                angval2 = Calc.getAngle(arc2.x, arc2.y, x, y);
    ...
    static _crossArcs(ctx, arc1, angval, x, y) {
        ...
        if (arc1.isCircle) {
            let a1 = arc1.clone();
            a1.angle = new Angle(angval, angval + DVA_PI);
            resArcs.push(a1);
        }
        else {
