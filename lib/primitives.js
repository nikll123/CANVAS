...
class Point extends ObjGraf {
    ...
    _render(ctx, radius = 0, color, draft = false) {
        ...
        if (draft) {
            var text = this.toString(FORMAT.SHORT);
...
class Angle {
    ...
    render(ctx, x, y, radius = 0, color = DEFAULT_COLOR) {
        this._render(ctx, x, y, radius, color);
    }
    draft(ctx, x, y, radius = 10, color = DRAFT_COLOR) {
        this._render(ctx, x, y, radius, color, DRAFT);
    }

    _render(ctx, x, y, radius, color, draft = false) {
        if (radius > 0) {
            ctx.save();
