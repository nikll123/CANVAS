...
class Arc extends BasicShape {
    ...
    clone() {
        if (this.type == 'Arc')
            ...
            return new Circle(this.x, this.y, this.radius, this.ccw, id + '_clone');
        else
    ...
    splitXY(ctx, x, y) {
        console.assert(isPointInStroke(ctx, x, y));
        let resArcs = [];
...
class Circle extends Arc {
    ...
    convertToArc() {
        return new Arc(this.x, this.y, this.radius, this.angle, this.ccw, this.id + '_arc')
    }

    splitXY(ctx, x, y) {
        console.assert(this.isPointInStroke(ctx, x, y));
        let a = Calc.getAngle(this.x, this.y, x, y);
        let resArcs = [];
        let arc = this.convertToArc();
        let a1 = new Angle(a, a + this.ccw_coeff * DVA_PI);
        arc.angle = a1;
        resArcs.push(arc);
        return resArcs;
    }
