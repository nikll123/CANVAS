...
class CalcCross {
    ...
    static crossVectorArc(ctx, vec, arc) {
        ...
        if (h < arc.radius) {
            ...
            resArc.angle = ang1;
            let begX = resArc.beginX;
            let begY = resArc.beginY;
            let endX = resArc.endX;
            let endY = resArc.endY;
            ...
            if (isBegOn && isEndOn) {
                ...
                // resVec = vec.dropMiddle(ctx, begX, begY, endX, endY);
                // resOl.push(resVec[0]);
                // resOl.push(resArc);
                // resOl.push(resVec[1]);
            }
                ...
                if (isBegOn) {
                    // resVec = vec.cutBegin(ctx, begX, begY)
                    // resOl.push(resVec[0]);
                    resArc.end = resArc.begin + resArc.ccw_coeff * DVA_PI;
                    // resOl.push(resArc);
                }
                else { //isEndOn
                    resArc.begin = resArc.end - resArc.ccw_coeff * DVA_PI;
                    // resOl.push(resArc);
                    // resVec = vec.cutEnd(ctx, endX, endY)
                    // resOl.push(resVec[0]);
                }
            }
        }
            ...
            // resVec = vec.cut(ctx, hx, hy)
            // resOl.push(resVec[0]);
            resArc.begin = h_angle;
            resArc.end = resArc.begin + DVA_PI;
            // resOl.push(resArc);
            // resOl.push(resVec[1]);
        }
        else {
        ...
        for (let i = 0; i < ol.lines.length; i++) {
            ...
            switch (l.type) {
                case 'Arc':
                    ...
                    break;
                case 'Vector':
                    [reltype, ol1, arc1] = this.crossVectorArc(ctx, l, arc);
                    arc = arc1;
                    ol1.draft(ctx);
                    break;
                default:
