...
class CalcCross {
    ...
    static crossVectorArc(ctx, arc, vec) {
        console.assert(arc.angle.value <= DVA_PI)
        let crossType = LINEPNTPOS_NONE;
        let a = arc.clone();
        let resOl = new Outline();
        let vecRes = [vec.clone()];
        let h, hx, hy, h_angle, pos;
        [h, hx, hy, h_angle, pos] = Calc.getAltitudeTriangle(vec.x, vec.y, vec.x2, vec.y2, a.x, a.y);
        if (h < arc.radius) {
            ...
            if (h == 0) {
                ...
                let a1 = Math.acos(Math.abs(h) / a.radius);
                if (pos == LINEPNTPOS_LEFT)
                    a1 = - a1;
                a_tmp1.substract(a1);
                a_tmp2.add(a1);
            }
            a.begin = a_tmp1.value;
            a.end = a_tmp2.value;
            let begX = a.beginX;
            let begY = a.beginY;
            let endX = a.endX;
            let endY = a.endY;
            let dBeg = Calc.getDistance(vec.x, vec.y, begX, begY);
            let dEnd = Calc.getDistance(vec.x, vec.y, endX, endY);
            if (dEnd < dBeg)
                a.angle.reverse();
            a.normalize();
            let isBegOn = vec.isPointInStroke(ctx, begX, begY);
            let isEndOn = vec.isPointInStroke(ctx, endX, endY);
            if (isBegOn && isEndOn) {
                crossType = RELATION_CROSS_2;
                vecRes = vec.dropMiddle(ctx, begX, begY, endX, endY);
                resOl.push(vecRes[0]);
                resOl.push(a);
                resOl.push(vecRes[1]);
            }
                ...
                if (isBegOn) {
                    vecRes = vec.cutBegin(ctx, begX, begY)
                    resOl.push(vecRes[0]);
                    a.end = a.begin + DVA_PI;
                    resOl.push(a);
                }
                else { //isEndOn
                    a.begin = a.end - DVA_PI;
                    resOl.push(a);
                    vecRes = vec.cutEnd(ctx, endX, endY)
                    resOl.push(vecRes[0]);
                }
            }
        }
    ...
    static crossArcArc(ctx, arcMain, arcSecond) {
        ...
        if (distance > r1 + r2) {
            ...
            let arc1_begIsOn = arcSecond.isPointInStroke(ctx, crossArcMain.beginX, crossArcMain.beginY);
            let arc1_endIsOn = arcSecond.isPointInStroke(ctx, crossArcMain.endX, crossArcMain.endY);
            ...
            if (arc1_begIsOn && arc1_endIsOn) {
                ...
                if (arcMain.isCircle) {
                    ...
                    resArcs1 = arcMain.splitAngs([crossArcMain.begin, crossArcMain.begin]);
        ...
        if (this.doesCircleCrossVector(ctx, ccl, vector)) {
            ...
            if (h < ccl.radius) {
                ...
                if (beginIsOnLine && endIsOnLine) {
                    ...
                    vecRes = vector.dropMiddle(ctx, arc1.beginX, arc1.beginY, arc1.endX, arc1.endY);
                }
