...
class BasicShape extends Obj {
    constructor(pointsArg, id = 'bs') {
        super(id)  // 0 0 - dummy values, will be substituted with points[0]
        this.points = [];
    ...
    toString() {
        return this.type + '-' + this.id + ': x=' + Calc.round(this.x) + '; y=' + Calc.round(this.y);
    }
...
class LineBroken extends BasicShape {
    ...
    toString() {
        var reval = this.constructor.name + '-' + this.id + ': '
        for (var i = 0; i < this.points.length; i++) {
