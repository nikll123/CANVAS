...
class Calc {
    ...
    static crossCircleLine(ctx, arc, line) {
        ...
        if (hm >= arc.radius) { // no overlapping
            ...
            let a_tmp1 = new Angle1(angle_base);
            a_tmp1.substract(a1);
            let a_tmp2 = new Angle1(angle_base);
            a_tmp2.add(a1);
            angle1 = new Angle(a_tmp1.value, a_tmp2.value);
            let p = new Path2D();
            p.moveTo(line.x, line.y);
    ...
    static crossCircleOutLine(c, ol) {
        ...
        for (let i = 0; i < ol.lines.length; i++) {
            let l = ol.lines[i];
            if (l.type == 'Vector') {
                let ret = this.crossCircleLine(ctx,c, l);
                if (ret.length > 0) {
                    angles.push(ret[0]);
                    angles.push(ret[1]);
                }
        ...
        for (let i = 0; i < ol1.lines.length; i++) {
            ...
            for (let j = 0; j < ol2.lines.length; j++) {
                ...
                if (l1.type == 'Arc' && l2.type == 'VectorComposite') {
                    let ret = this.crossCircleLine(ctx, l1, l2);
                    if (ret.length > 0) {
                        angles.push(ret[0]);
                        angles.push(ret[1]);
                    }
