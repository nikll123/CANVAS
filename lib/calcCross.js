...
class CalcCross {
    ...
    static arcCross(ctx, arc1, arc2) {
        ...
        if (distance > r1 + r2) {
            ...
            let crossAngle_arc1 = CalcCross.getCrossAngle(arc1, arc2);
            let crossArc_c1 = new Arc(arc1.x, arc1.y, arc1.radius, crossAngle_arc1, arc1.ccw);
            let c1_begIsOn = arc2.isPointInStroke(ctx, crossArc_c1.beginX, crossArc_c1.beginY);
            let c1_endIsOn = arc2.isPointInStroke(ctx, crossArc_c1.endX, crossArc_c1.endY);
            ...
            if (c1_begIsOn && c1_endIsOn) {
                ...
                if (arc1.isCircle) {
                    let a1 = arc1.clone();
                    a1.angle = new Angle(crossArc_c1.begin, crossArc_c1.end);
                    resArcs1.push(a1);
                    let a2 = arc1.clone();
                    a2.angle = new Angle(crossArc_c1.end, crossArc_c1.begin);
                    resArcs1.push(a2);
                }
                else {
                    [a11, a12] = arc1.splitXY(ctx, crossArc_c1.beginX, crossArc_c1.beginY);
                    [a12, a13] = a12.splitXY(ctx, crossArc_c1.endX, crossArc_c1.endY);
                    resArcs1.push(a11);
                    resArcs1.push(a12);
                    resArcs1.push(a13);
                }
                resArcs2 = [];
                let a21, a22, a23;
                [a21, a22] = arc2.splitXY(ctx, crossArc_c1.endX, crossArc_c1.endY);
                // arc2.render(ctx, 6,'#0000FF');
                // a22.render(ctx, 3,'#00FF00');
                [a22, a23] = a22.splitXY(ctx, crossArc_c1.beginX, crossArc_c1.beginY);
                resArcs2.push(a21);
                resArcs2.push(a22);
                resArcs2.push(a23);
            }
                ...
                let x, y;
                if (c1_begIsOn)
                    [x, y] = [crossArc_c1.beginX, crossArc_c1.beginY];
                else
                    [x, y] = [crossArc_c1.endX, crossArc_c1.endY];
                resArcs1 = [];
                let a1, a2;
    ...
    static getCrossAngle(arc1, arc2) {
        let r1 = arc1.radius;
        let r2 = arc2.radius;
        let distance = Calc.getDistance(arc1.x, arc1.y, arc2.x, arc2.y);
