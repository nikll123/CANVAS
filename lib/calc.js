...
class Calc {
    ...
    static crossCircleLine(arc, line) {
        let a_tmp = Calc.getAngle(line.x, line.y, arc.x, arc.y);
        let angle_l = Calc.getAngle(line.x, line.y, line.x2, line.y2);
        let a1 = Math.abs(angle_l - a_tmp);
        let distance = Calc.round(this.getDistance(line.x, line.y, arc.x, arc.y));
        ...
        if (hm >= arc.radius) {
            ...
            let hx = Calc.round(line.x + cath * Math.cos(angle_l));
            let hy = Calc.round(line.y + cath * Math.sin(angle_l));

            let angle_base = Calc.getAngle(arc.x, arc.y, hx, hy);
            a1 = Math.acos(hm / arc.radius);
            let a_tmp1 = new Angle(0, angle_base);
            a_tmp1.substract(a1);
            let a_tmp2 = new Angle(0, angle_base);
            a_tmp2.add(a1);
            if (a_tmp2.end > DVA_PI)
            {
                a_tmp2.rayEnd.normalize();
                a_tmp1.begin = a_tmp2.end;
                a_tmp2.begin = DVA_PI;
                a_tmp2.end = DVA_PI;
            }
            let angle1, angle2;
            let arc1, arc2;
            if (h >= 0) {
                angle1 = new Angle(a_tmp1.begin, a_tmp1.end);
                angle2 = new Angle(a_tmp2.begin, a_tmp2.end);
            }
            else {
                angle1 = new Angle(a_tmp1.end, a_tmp2.end);
                angle2 = new Angle(a_tmp2.end, a_tmp2.end);
            }

            arc1 = new Arc(arc.x, arc.y, arc.radius, angle1);
            arc2 = new Arc(arc.x, arc.y, arc.radius, angle2);
            let pnt1IsOnLine = Calc.lineOwnsPoint(line, arc1.endX, arc1.endY);
            let pnt2IsOnLine = Calc.lineOwnsPoint(line, arc2.endX, arc2.endY);
    ...
    static crossCircleOutLine(c, ol) {
        ...
        if (angles.length == 0) {
            ...
            angles.push(new Angle(DVA_PI, DVA_PI));
        }
        if (angles.length == 4) {
        ...
        for (let i = 0; i < ol1.lines.length; i++) {
            ...
            for (let j = 0; j < ol2.lines.length; j++) {
                ...
                if (l1.type == 'Arc' && l2.type == 'VectorComposite') {
                    let ret = this.crossCircleLine(l1, l2);
                    if (ret.length > 0) {
                        angles.push(ret[0]);
                        angles.push(ret[1]);
                    }
