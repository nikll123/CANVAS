...
class Calc {
    ...
    static crossCircleOutLine(c, ol) {
        ...
        if (this.doesCircleCrossline(ctx, c, ol)) {
            let centreInPath = ol.isPointInPath(ctx, c.x, c.y);
            for (let i = 0; i < ol.lines.length; i++) {
                const l = ol.lines[i];
                let res = [];
                if (l.type == 'Vector') {
                    if (this.doesCircleCrossVector(ctx, c, l)) {
                        let angle1, x1, y1, x2, y2;
                        [angle1, x1, y1, x2, y2] = this.crossCircleVector(ctx, c, l, centreInPath);
                        if (angle1 != undefined) {
                            angles.push(angle1);
                            let new_l = [];
                            if (x1 != undefined)
        ...
        // for (let i = 0; i < ol.lines.length; i++) {
        //     ol.lines[i].beginX = Calc.round(ol.lines[i].beginX);
        //     ol.lines[i].beginY = Calc.round(ol.lines[i].beginY);
        //     ol.lines[i].endX = Calc.round(ol.lines[i].endX);
        //     ol.lines[i].endY = Calc.round(ol.lines[i].endY);
        // }

        // for (let i = ol.lines.length - 1; i > 0; i--) {
        //     const line1 = ol.lines[i];
        //     const line2 = ol.lines[i - 1];
        //     if (line1.x != line2.x2 || line1.y != line2.y2) {
        //         ol.lines.splice(i, 0, new Vector(line2.x2, line2.y2, line1.x, line1.y));
        //     }
        // }

        if (angles.length == 2) {
            if (angles[0].isDVA_PI && angles[1].isDVA_PI) {
                angles = [new Angle(angles[0].radians1, angles[1].radians2)]
            }
        }
        return [angles, ol];
    }
